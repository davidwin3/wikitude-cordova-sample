...
var defaultScaleImageValue = 0.8;
...
var World = {
    ...
    modelPaths: [
        {
            type: 'IMAGE',
            path: "assets/images/vangogh.jpg",
        },
        {
            type: 'VIDEO',
        ...
        {
            type: 'MODEL',
            path: "assets/models/officechair.wt3",
        },
        {
            type: 'MODEL',
            path: "assets/models/table.wt3",
        },
        {
            type: 'MODEL',
            path: "assets/models/trainer.wt3"
        }
    ],
    /*
    ...
    */
    ...
    addModel: function addModelFn(pathIndex, xpos, ypos) {
        if (World.isTracking()) {
            ...
            var options = {
                scale: {
                    x: World.modelPaths[pathIndex].type === 'MODEL' ? defaultScaleValue : defaultScaleImageValue,
                    y: World.modelPaths[pathIndex].type === 'MODEL' ? defaultScaleValue : defaultScaleImageValue,
                    z: World.modelPaths[pathIndex].type === 'MODEL' ? defaultScaleValue : defaultScaleImageValue,
                },
                translate: {
                    x: xpos,
                    y: ypos
                ...
                rotate: {
                    x: World.modelPaths[pathIndex].type === 'MODEL' ? defaultRotationValue : -90,
                },
                /*
            ...
            var model = World.makeModel(World.modelPaths[pathIndex], options);
    ...
    makeModel: function makeModelFn(model, options) {
        switch (model.type) {
            case 'MODEL':
                return new AR.Model(model.path, options);
            case 'IMAGE':
                return new AR.ImageDrawable(new AR.ImageResource(model.path), 0.6, options)
            case 'VIDEO':
                World.video = new AR.VideoDrawable('assets/videos/video.mp4', 0.4, options);
                return World.video;
        }
    },
